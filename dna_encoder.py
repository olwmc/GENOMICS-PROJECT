...
class DNAEncoder(nn.Module):
    """Transformer encoder for DNA sequences with proper RPE"""
    def __init__(self, seq_len=100, embedding_dim=256, d_model=128, 
                 num_heads=4, num_layers=3, dim_feedforward=512, dropout=0.1):
        super().__init__()
        self.d_model = d_model
        self.seq_len = seq_len
        
        # Convert 4-channel one-hot to embeddings
        self.input_projection = nn.Linear(4, d_model)
        
        # Transformer blocks with RPE
        self.blocks = nn.ModuleList([
            TransformerBlockWithRPE(
                d_model, num_heads, dim_feedforward, dropout, max_distance=seq_len
            )
...
# Alternative: Simpler approach with absolute positional encoding
class DNAEncoderSimple(nn.Module):
    """Transformer encoder with absolute positional encoding (often works better)"""
    def __init__(self, seq_len=100, embedding_dim=256, d_model=128, 
                 num_heads=4, num_layers=3, dim_feedforward=512, dropout=0.1):
        super().__init__()
        self.d_model = d_model
    ...
    def forward(self, x):
        # x shape: [batch, 4, seq_len]
        x = x.transpose(1, 2)  # [batch, seq_len, 4]
